...
class Readability3 {
    ...
    function __construct($source, $input_char = "utf-8") {
        ...
        try {
            ...
            // 会有些错误信息，不过不要紧 :^)
            if (!@$this->DOM->loadHTML('<?xml encoding="'.Readability3::DOM_DEFAULT_CHARSET.'">'.$source)) {
            ...
            // insert proper
            $this->DOM->encoding = Readability3::DOM_DEFAULT_CHARSET;
        } catch (Exception $e) {
            // ...
        }
    }
    ...
    private function getTopBox() {
        ...
        while($paragraph = $allParagraphs->item($i++)) {
            ...
            $contentScore = intval($parentNode->getAttribute(Readability3::ATTR_CONTENT_SCORE));
            ...
            // 保存父元素的判定得分
            $parentNode->setAttribute(Readability3::ATTR_CONTENT_SCORE, $contentScore);
        ...
        for ($i = 0, $len = sizeof($this->parentNodes); $i < $len; $i++) {
            ...
            $contentScore    = intval($parentNode->getAttribute(Readability3::ATTR_CONTENT_SCORE));
            $orgContentScore = intval($topBox ? $topBox->getAttribute(Readability3::ATTR_CONTENT_SCORE) : 0);
    ...
    public function getContent() {
        ...
        if($ContentBox === null)
            throw new \RuntimeException(Readability3::MESSAGE_CAN_NOT_GET);
        ...
        return Array(
            ...
            'content' => mb_convert_encoding($Target->saveHTML(), Readability3::DOM_DEFAULT_CHARSET, "HTML-ENTITIES")
        );
    }
